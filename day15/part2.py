...
def hash(s):
    return functools.reduce(hash_op, s, 0)
...
def main():
    ...
    for instr in instructions:
        ...
        label = m[1]
        op = m[2]
        value = int(m[3]) if m[3] else None
        i = hash(label)
        if op == '=':
            if any(l == label for l, _ in boxes[i]):
                boxes[i] = [(label, value) if l == label else (l, v) for l, v in boxes[i]]
            else:
                boxes[i].append((label, value))
        if op == '-':
            boxes[i] = [(l, v) for l, v in boxes[i] if l != label]

    powers = [
        [(i + 1) * j * v for j, (_, v) in enumerate(box, 1)]
        for i, box in enumerate(boxes)
    ]
    pprint.pprint([(i, x) for i, x in enumerate(powers) if x])
